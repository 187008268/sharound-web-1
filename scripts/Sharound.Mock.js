...
/**
 * Adds a set of mock Items to the Cloud Firestore.
 */
Sharound.prototype.addMockItems = function() {
  var promises = [];
  ...
  for (var i = 0; i < 20; i++) {
    ...
    if (!promise) {
      alert('addItem() is not implemented yet!');
...
/**
 * Adds a set of mock Ratings to the given Item.
 */
Sharound.prototype.addMockRatings = function(itemID) {
  var ratingPromises = [];
  for (var r = 0; r < 5*Math.random(); r++) {
    ...
    ratingPromises.push(this.addRating(itemID, rating));
  }
